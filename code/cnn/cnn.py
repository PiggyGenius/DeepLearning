...
# ************************************************************************************************ #
#                                                                                                  #
# The input is 28x28 grayscale images, the input data goes through two convolutional layers that   #
# have a kernel size of 5x5.                                                                       #
# The first convolution has 32 output feature maps and the second one 64 output feature maps.      #
# Each convolution layer is followed by a subsampling layer, max-pooling operation.                #
# Then, a fully-connected layer passes the output to a second fully connected layer.               #
# The second fully-connected layer acts as the final softmax output layer.                         #
#                                                                                                  #
#   - input: [batchsize x 28 x 28 x 1]                                                             #
#   - conv_1: [batchsize x 24 x 24 x 32]                                                           #
#   - pooling_1: [batchsize x 12 x 12 x 32]                                                        #
#   - conv_2: [batchsize x 8 x 8 x 64]                                                             #
#   - pooling_2: [batchsize x 4 x 4x 64]                                                           #
...
# We create a convolutional layer
def conv_layer(input_tensor, name, kernel_size, n_output_channels, padding_mode='SAME', strides=(1, 1, 1, 1)):
    with tf.variable_scope(name):
        ...
        weights = tf.get_variable(name='_weights', shape=weights_shape) # Xavier init by default
...
# We create our fully connected layers
